...
//function to call to dynamically call the on click for buttons. When the user clicks a button, empty the div with an id of "display". Create a variable p that is equal to the clicked buttons data-name attribute. Create a variable queryURL that is the link to the giphyAPI and has the variable p passed into it's search query. Use jQuery to call ajax. Pass ajax queryURL, tell it to 'GET' a JSON object. When the object have been retrieved create a variable results that is equal to the object.data key. Loop through results. If results.rating has a rating of pg-13 or r then do nothing. Else create a variable gifDiv that is equal to a jQuery div object with a class of "item" and "col-md-4". Create variable rating and set it equal to the current index of results.rating. Create variable p and set it equal to a jQuery created p tag with a text of '"rating"+ the rating variable'. Create variable gifImage and make it equal to a jQuery created img tag. Add a src attribute to gifImage that is equal to results[i].images.fixed_height_still.url. Add a data-animate attribute that contains the link for the animated gif. Add another data-still attribute that contains the link the still gif. Append the variable p the variable gifDiv. Append the variable gifImage to the variable gifDiv. Prepend gifDiv to the div an id of display. Pass the variable gifImage to the function imgClick so that the on click works. 
function clicks() {
